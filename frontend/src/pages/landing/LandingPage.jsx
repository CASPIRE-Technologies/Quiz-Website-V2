...
export default function LandingPage() {
  ...
  // Platform stats
  const platformStats = [
    { label: isSinhala ? 'ක්‍රියාකාරී සිසුන්' : 'Active Students', value: '15,000+', icon: Users, color: '#38BDF8' },
    { label: isSinhala ? 'සම්පූර්ණ කළ ප්‍රශ්නාවලි' : 'Quizzes Completed', value: '120,000+', icon: Trophy, color: '#F59E0B' },
    { label: isSinhala ? 'විභාග සාමාර්ථ්‍ය ප්‍රතිශතය' : 'Success Rate', value: '94.8%', icon: Zap, color: '#10B981' },
    { label: isSinhala ? 'විශේෂඥ ආදර්ශ ප්‍රශ්න පත්‍ර' : 'Expert Model Papers', value: '500+', icon: BookOpen, color: '#8B5CF6' },
  ];
  ...
  const examTracks = [
    ...
    {
      id: 'ol',
      title: isSinhala ? 'අ.පො.ස. සාමාන්‍ය පෙළ (O/L)' : 'G.C.E. Ordinary Level (O/L)',
    ...
    {
      id: 'al',
      title: isSinhala ? 'අ.පො.ස. උසස් පෙළ (A/L)' : 'G.C.E. Advanced Level (A/L)',
  ...
  const coreFeatures = [
    {
      title: isSinhala ? 'ක්ෂණික ලකුණු හා විශ්ලේෂණ' : 'Instant Scoring & Analytics',
      desc: isSinhala
    ...
    {
      title: isSinhala ? 'ප්‍රවීණ ආචාර්ය මණ්ඩලයක් විසින් සම්පාදිතයි' : 'Curated by Top Educators',
      desc: isSinhala
    ...
    {
      title: isSinhala ? 'ද්විභාෂා විවරණ (සිංහල සහ ඉංග්‍රීසි)' : 'Bilingual Solutions',
      desc: isSinhala
  ...
  const plansData = [
    {
      ...
      desc: isSinhala ? 'වේදිකාව අත්හදා බැලීමට කැමති සිසුන් සඳහා ආරම්භක ප්‍රවේශය.' : 'Great for trying out sample quizzes and discovering the platform.',
      features: [
        isSinhala ? 'ආදර්ශ ප්‍රශ්නාවලි 3ක් සඳහා නොමිලේ ප්‍රවේශය' : 'Access to 3 free sample model papers',
        isSinhala ? 'ක්ෂණික ලකුණු පුවරුව හා නිවැරදි පිළිතුරු' : 'Instant scoring & basic answer view',
        isSinhala ? 'ප්‍රජා සාකච්ඡා සහ ප්‍රශ්න විසඳුම්' : 'Community forum discussions',
        isSinhala ? 'ඕනෑම උපාංගයකින් ප්‍රවේශ වීම' : 'Standard web & mobile access'
      ],
      isPopular: false,
    ...
    },
    {
      ...
      period: billingCycle === 'term' ? (isSinhala ? 'වාරයකට (මාස 3)' : 'per term (3 mos)') : (isSinhala ? 'වසරකට' : 'per academic year'),
      desc: isSinhala ? 'විභාගයෙන් ඉහළම ප්‍රතිඵල අපේක්ෂා කරන බුද්ධිමත් සිසුන් සඳහා.' : 'Comprehensive exam preparation with full access to all past & model papers.',
      features: [
        isSinhala ? 'තෝරාගත් ශ්‍රේණියේ සියලුම ආදර්ශ හා පසුගිය ප්‍රශ්න පත්‍ර' : 'Full access to all model & past papers',
        isSinhala ? 'සීමාරහිත ප්‍රශ්නාවලි උත්සාහයන් (Unlimited Attempts)' : 'Unlimited timed quiz attempts',
        isSinhala ? 'පියවරෙන් පියවර විස්තරාත්මක පිළිතුරු විවරණ' : 'Complete step-by-step solutions',
        isSinhala ? 'AI දුර්වලතා හඳුනාගැනීමේ ප්‍රස්ථාර හා විශ්ලේෂණ' : 'AI progress radar & weakness tracking',
        isSinhala ? 'දිවයිනේ ශ්‍රේණිගත කිරීම් ලීඩර්බෝඩ් ප්‍රවේශය' : 'Island-wide student leaderboard rankings'
      ],
      isPopular: true,
    ...
    },
    {
      ...
      period: billingCycle === 'term' ? (isSinhala ? 'වාරයකට' : 'per term') : (isSinhala ? 'වසරකට' : 'per academic year'),
      desc: isSinhala ? 'A සාමාර්ථ්‍යයන් හා දිවයිනේ කුසලතා ඉලක්ක කරන සිසුන් සඳහා සම්පූර්ණ විසඳුම.' : 'The all-inclusive elite training suite for top island & district rank aspirants.',
      features: [
        isSinhala ? 'Student Pro හි සියලුම පහසුකම්' : 'Everything in Student Pro',
        isSinhala ? 'මුද්‍රණය කළ හැකි කෙටි සටහන් හා PDF ප්‍රශ්න පත්‍ර' : 'Downloadable PDF revision notes & summaries',
        isSinhala ? 'අවසන් විභාග ඉලක්කගත විශේෂ අනුමාන ප්‍රශ්න පත්‍ර' : 'Exclusive final exam prediction papers',
        isSinhala ? 'WhatsApp හරහා ප්‍රවීණ ගුරු සහයෝගය' : 'Priority WhatsApp teacher Q&A support',
        isSinhala ? 'දෙමාපියන් සඳහා සතිපතා ප්‍රගති වාර්තා' : 'Weekly parent progress reports & SMS alerts'
      ],
      isPopular: false,
  ...
  const previewQuestions = {
    g5: {
      trackName: isSinhala ? '5 වසර ශිෂ්‍යත්වය - බුද්ධි පරීක්ෂණය' : 'Grade 5 Scholarship - IQ Logic',
      question: isSinhala
        ? 'රූප රටාවේ මීළඟට පැමිණිය යුතු රූපය කුමක්ද? [🔺, 🔻, 🔺, 🔻, ?]'
        : 'In a logic sequence: 2, 6, 12, 20, 30, ... What is the next number?',
      options: isSinhala 
        ? ['🔺 උඩුකුරු ත්‍රිකෝණය', '🔻 යටිකුරු ත්‍රිකෝණය', '🟦 නිල් සමචතුරස්‍රය', '⚪ සුදු වෘත්තය']
        : ['36', '40', '42', '44'],
    ...
    ol: {
      trackName: isSinhala ? 'අ.පො.ස. සාමාන්‍ය පෙළ - විද්‍යාව' : 'G.C.E. O/L - Science',
      question: isSinhala
    ...
    al: {
      trackName: isSinhala ? 'අ.පො.ස. උසස් පෙළ - සාමාන්‍ය පොදු පරීක්ෂණය' : 'G.C.E. A/L - General Knowledge & Aptitude',
      question: isSinhala
      ...
      options: isSinhala
        ? ['ඔක්සිජන් (O₂)', 'නයිට්‍රජන් (N₂)', 'කාබන් ඩයොක්සයිඩ් (CO₂)', 'ආගන් (Ar)']
        : ['Absolute Temperature', 'Total Pressure', 'Molar Mass', 'Gas Volume'],
  ...
  const faqItems = [
    {
      q: isSinhala ? 'Edu Pulse ප්‍රශ්නාවලි සකස් කරන්නේ කවුරුන් විසින්ද?' : 'Who prepares the questions and model papers on Edu Pulse?',
      a: isSinhala
    ...
    {
      q: isSinhala ? 'මට ජංගම දුරකථනයෙන් ප්‍රශ්න පත්‍ර වලට පිළිතුරු සැපයිය හැකිද?' : 'Can I take quizzes on my smartphone or tablet?',
      a: isSinhala
    ...
    {
      q: isSinhala ? 'ගෙවීම් සිදුකළ හැක්කේ කෙසේද?' : 'What payment methods are supported in Sri Lanka?',
      a: isSinhala
    ...
    {
      q: isSinhala ? 'නොමිලේ අත්හදා බැලීමට ක්‍රමයක් තිබේද?' : 'Can I try sample quizzes before paying?',
      a: isSinhala
      ...
      >
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', textAlign: 'center', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{
            ...
            <Sparkles size={15} />
            <span>{isSinhala ? 'ශ්‍රී ලංකාවේ බුද්ධිමත්ම විභාග සූදානම් කිරීමේ වේදිකාව' : 'The Next-Gen Sri Lankan Exam Preparation Platform'}</span>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', flexWrap: 'wrap', marginBottom: '50px' }}>
            ...
            >
              <span>{user ? (isSinhala ? 'මගේ උපකරණ පුවරුව' : 'Go to Dashboard') : (isSinhala ? 'ප්‍රශ්නාවලි ආරම්භ කරන්න' : 'Start Practice Free')}</span>
              <ArrowRight size={18} />
            </button>
            ...
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                document.getElementById('previews')?.scrollIntoView({ behavior: 'smooth' });
              }}
              style={{
                display: 'inline-flex',
                alignItems: 'center',
            ...
            >
              ...
              <span>{isSinhala ? 'සජීවී ආදර්ශ ප්‍රශ්නයක් බලන්න' : 'Try Live Question Preview'}</span>
            </a>
          </div>
          ...
          <div style={{
            ...
            boxShadow: isDark ? '0 10px 30px -10px rgba(0,0,0,0.5)' : '0 12px 28px -8px rgba(15, 23, 42, 0.08)',
            ...
            {platformStats.map((stat, idx) => {
              ...
              return (
                <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '14px', textAlign: 'left' }}>
                  <div style={{
                    width: '46px',
                    height: '46px',
                    borderRadius: '12px',
                  ...
                  <div>
                    <div style={{ fontSize: '22px', fontWeight: 800, color: isDark ? '#FFFFFF' : '#0F172A', lineHeight: 1.1 }}>
                      {stat.value}
                    </div>
                    <div style={{ fontSize: '12px', color: isDark ? '#94A3B8' : '#64748B', fontWeight: 500, marginTop: '3px' }}>
                      {stat.label}
                    </div>
                  </div>
                </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '22px', textAlign: 'left' }}>
            {examTracks.map((track) => (
              <div
                ...
                style={{
                  ...
                  position: 'relative',
                  overflow: 'hidden',
                  boxShadow: isDark ? '0 10px 25px -8px rgba(0,0,0,0.4)' : '0 8px 20px -6px rgba(0,0,0,0.06)',
              ...
              >
                ...
                <h3 style={{ fontSize: '19px', fontWeight: 700, color: isDark ? '#FFFFFF' : '#0F172A', marginBottom: '10px' }}>
                  {track.title}
                </h3>
                <p style={{ fontSize: '14px', color: isDark ? '#94A3B8' : '#64748B', lineHeight: 1.55, marginBottom: '22px' }}>
                  {track.desc}
                </p>

                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px', fontWeight: 700, color: '#0284C7' }}>
                  <span>{isSinhala ? 'ප්‍රශ්න පත්‍ර බලන්න' : 'Explore Practice Sets'}</span>
      ...
      >
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '50px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(26px, 4vw, 36px)', fontWeight: 800, color: isDark ? '#FFFFFF' : '#0F172A', marginBottom: '12px' }}>
              {isSinhala ? 'විභාග සාමාර්ථ්‍යය තහවුරු කරන විශේෂාංග' : 'Designed for Academic Excellence'}
            </h2>
            <p style={{ color: isDark ? '#94A3B8' : '#64748B', fontSize: '16px', maxWidth: '600px', margin: '0 auto' }}>
              {isSinhala
              ...
              return (
                <div 
                  ...
                  style={{
                    ...
                    boxShadow: isDark ? '0 8px 20px rgba(0, 0, 0, 0.25)' : '0 6px 18px rgba(15, 23, 42, 0.04)',
                ...
                >
                  ...
                  <h4 style={{ fontSize: '18px', fontWeight: 700, color: isDark ? '#FFFFFF' : '#0F172A', marginBottom: '10px' }}>
                    {feat.title}
                  </h4>
                  <p style={{ fontSize: '14px', color: isDark ? '#94A3B8' : '#64748B', lineHeight: 1.6, margin: 0 }}>
                    {feat.desc}
                  </p>
                </div>
              );
      ...
      <section id="plans" style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(26px, 4vw, 36px)', fontWeight: 800, color: isDark ? '#FFFFFF' : '#0F172A', marginBottom: '12px' }}>
              {isSinhala ? 'ඔබට ගැළපෙන හොඳම පැකේජය තෝරාගන්න' : 'Transparent Student Pricing Plans'}
            </h2>
            <p style={{ color: isDark ? '#94A3B8' : '#64748B', fontSize: '16px', maxWidth: '620px', margin: '0 auto 28px' }}>
              {isSinhala
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  transition: 'all 0.2s ease',
                  backgroundColor: billingCycle === 'term' ? '#0284C7' : 'transparent',
                  color: billingCycle === 'term' ? '#FFFFFF' : isDark ? '#94A3B8' : '#64748B'
                }}
              >
              ...
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  transition: 'all 0.2s ease',
                  backgroundColor: billingCycle === 'year' ? '#0284C7' : 'transparent',
                  color: billingCycle === 'year' ? '#FFFFFF' : isDark ? '#94A3B8' : '#64748B'
                }}
              >
                <span>{isSinhala ? 'වාර්ෂික ගෙවීම්' : 'Full Academic Year'}</span>
                <span style={{
                  fontSize: '10px',
                  fontWeight: 800,
                  background: '#10B981',
            ...
            {plansData.map((plan) => (
              <div
                ...
                style={{
                  ...
                  boxShadow: plan.isPopular
                    ...
                    : isDark ? '0 10px 25px -8px rgba(0,0,0,0.3)' : '0 8px 22px rgba(15, 23, 42, 0.05)',
              ...
              >
                ...
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <p style={{ fontSize: '13px', color: isDark ? '#94A3B8' : '#64748B', lineHeight: 1.5, minHeight: '40px' }}>
                    {plan.desc}
                  </p>
                </div>

                <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', marginBottom: '24px' }}>
                  <span style={{ fontSize: '34px', fontWeight: 900, color: isDark ? '#FFFFFF' : '#0F172A' }}>
                    {plan.price}
                  </span>
                ...
                <div style={{ flex: 1, marginBottom: '28px' }}>
                  <div style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', color: isDark ? '#CBD5E1' : '#475569', marginBottom: '14px' }}>
                    {isSinhala ? 'ඇතුළත් විශේෂාංග:' : 'Included Features:'}
                  </div>
                  <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {plan.features.map((feat, fIdx) => (
                      <li key={fIdx} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', fontSize: '14px', color: isDark ? '#E2E8F0' : '#334155' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: '14px',
                    borderRadius: '12px',
                    border: plan.isPopular ? 'none' : isDark ? '1px solid rgba(255, 255, 255, 0.15)' : '1px solid rgba(0, 0, 0, 0.15)',
                    background: plan.isPopular
                      ? 'linear-gradient(135deg, #0284C7 0%, #2563EB 100%)'
                      : 'transparent',
                    color: plan.isPopular ? '#FFFFFF' : isDark ? '#F1F5F9' : '#0F172A',
                    fontWeight: 700,
                    fontSize: '14px',
                    cursor: 'pointer',
                    boxShadow: plan.isPopular ? '0 6px 18px rgba(37, 99, 235, 0.35)' : 'none',
                    transition: 'all 0.2s ease'
                  }}
                  className="btn-interactive"
                >
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '48px', alignItems: 'center' }}>
            ...
            <div>
              ...
              <h2 style={{ fontSize: 'clamp(26px, 4vw, 36px)', fontWeight: 800, color: isDark ? '#FFFFFF' : '#0F172A', marginBottom: '16px', lineHeight: 1.25 }}>
                {isSinhala
                  ? 'සෑම ශ්‍රී ලාංකික සිසුවෙකුටම සමාන ගුණාත්මක අධ්‍යාපනයක්'
                  : 'Empowering Every Sri Lankan Student to Excel'}
              </h2>
              <p style={{ fontSize: '15px', color: isDark ? '#94A3B8' : '#64748B', lineHeight: 1.7, marginBottom: '20px' }}>
                {isSinhala
              ...
              <p style={{ fontSize: '15px', color: isDark ? '#94A3B8' : '#64748B', lineHeight: 1.7, marginBottom: '28px' }}>
                {isSinhala
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div style={{ padding: '16px', borderRadius: '14px', background: isDark ? '#1E293B' : '#FFFFFF', border: isDark ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(0,0,0,0.06)' }}>
                  <div style={{ fontSize: '24px', fontWeight: 800, color: '#0284C7' }}>9 Provinces</div>
                  <div style={{ fontSize: '12px', color: isDark ? '#94A3B8' : '#64748B' }}>Island-wide active reach</div>
                </div>
                <div style={{ padding: '16px', borderRadius: '14px', background: isDark ? '#1E293B' : '#FFFFFF', border: isDark ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(0,0,0,0.06)' }}>
                  <div style={{ fontSize: '24px', fontWeight: 800, color: '#10B981' }}>150+ Top Teachers</div>
                  <div style={{ fontSize: '12px', color: isDark ? '#94A3B8' : '#64748B' }}>Curriculum paper setters</div>
                </div>
              </div>
            </div>
            ...
            <div style={{
              ...
              border: isDark ? '1px solid rgba(255, 255, 255, 0.08)' : '1px solid rgba(0, 0, 0, 0.08)',
              padding: '36px 30px',
              boxShadow: isDark ? '0 12px 32px rgba(0,0,0,0.35)' : '0 10px 28px rgba(15, 23, 42, 0.06)'
            }}>
              ...
              <blockquote style={{ fontSize: '16px', fontStyle: 'italic', color: isDark ? '#E2E8F0' : '#334155', lineHeight: 1.7, marginBottom: '24px' }}>
                {isSinhala
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                ...
                <div>
                  <div style={{ fontSize: '15px', fontWeight: 700, color: isDark ? '#FFFFFF' : '#0F172A' }}>
                    Kaveesha Perera
                  </div>
      ...
      <section id="previews" style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: '960px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '36px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(26px, 4vw, 36px)', fontWeight: 800, color: isDark ? '#FFFFFF' : '#0F172A', marginBottom: '12px' }}>
              {isSinhala ? 'ප්‍රශ්නාවලි එන්ජිම සජීවීව අත්හදා බලන්න' : 'Experience the Quiz Engine Live'}
            </h2>
            <p style={{ color: isDark ? '#94A3B8' : '#64748B', fontSize: '16px', maxWidth: '580px', margin: '0 auto 28px' }}>
              {isSinhala
            ...
            <div style={{
              ...
              {[
                ...
                { id: 'ol', label: isSinhala ? 'සාමාන්‍ය පෙළ (O/L)' : 'G.C.E. O/L' },
                { id: 'al', label: isSinhala ? 'උසස් පෙළ (A/L)' : 'G.C.E. A/L' }
              ].map((tab) => (
                <button
                  key={tab.id}
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    transition: 'all 0.2s ease',
                    backgroundColor: selectedPreviewTrack === tab.id ? '#0284C7' : 'transparent',
                    color: selectedPreviewTrack === tab.id ? '#FFFFFF' : isDark ? '#94A3B8' : '#64748B'
                  }}
                >
                  {tab.label}
                </button>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '28px' }}>
              {currentPreview.options.map((option, idx) => {
                ...
                let optionBg = isDark ? 'rgba(255, 255, 255, 0.03)' : 'rgba(15, 23, 42, 0.02)';
                let optionBorder = isDark ? 'rgba(255, 255, 255, 0.08)' : 'rgba(0, 0, 0, 0.08)';
            ...
            {isAnswerChecked && (
              <div style={{
                borderRadius: '16px',
                padding: '20px 24px',
                backgroundColor: selectedOption === currentPreview.correctIndex ? 'rgba(16, 185, 129, 0.1)' : 'rgba(2, 132, 199, 0.1)',
                border: selectedOption === currentPreview.correctIndex ? '1px solid rgba(16, 185, 129, 0.3)' : '1px solid rgba(2, 132, 199, 0.3)',
                marginBottom: '26px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  ...
                  <span style={{ fontSize: '15px', fontWeight: 800, color: '#10B981' }}>
                    {selectedOption === currentPreview.correctIndex
                      ? (isSinhala ? 'නිවැරදියි! විශිෂ්ටයි!' : 'Correct! Great job!')
                      : (isSinhala ? 'නිවැරදි පිළිතුර විවරණය මෙන්න:' : 'Step-by-step Solution Explanation:')}
                  </span>
                </div>
                <p style={{ fontSize: '14px', color: isDark ? '#E2E8F0' : '#334155', lineHeight: 1.6, margin: 0 }}>
              ...
              {!isAnswerChecked ? (
                <button
                  ...
                  style={{
                    ...
                    boxShadow: selectedOption !== null ? '0 6px 18px rgba(37, 99, 235, 0.35)' : 'none'
                  }}
                >
                  <span>{isSinhala ? 'පිළිතුර පරීක්ෂා කරන්න' : 'Check Answer'}</span>
                  <ArrowRight size={16} />
                </button>
              ) : (
                <button
                ...
                >
                  <Sparkles size={16} />
                  <span>{isSinhala ? 'තවත් 500+ ප්‍රශ්නාවලි උත්සාහ කරන්න' : 'Try 500+ Full Model Papers'}</span>
                </button>
              )}
            </div>
          </div>
      ...
      >
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '50px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(26px, 4vw, 36px)', fontWeight: 800, color: isDark ? '#FFFFFF' : '#0F172A', marginBottom: '12px' }}>
              {isSinhala ? 'අපගේ අධ්‍යාපනික සහය කණ්ඩායම අමතන්න' : 'Get in Touch with Our Academic Team'}
            </h2>
            <p style={{ color: isDark ? '#94A3B8' : '#64748B', fontSize: '16px', maxWidth: '600px', margin: '0 auto' }}>
              {isSinhala
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '36px' }}>
            ...
            <div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '18px', marginBottom: '32px' }}>
                <div style={{
                  ...
                  <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: 'rgba(2, 132, 199, 0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <Phone size={20} color="#0284C7" />
                  </div>
                  <div>
                    <div style={{ fontSize: '12px', color: isDark ? '#94A3B8' : '#64748B' }}>Helpline & WhatsApp</div>
                    <div style={{ fontSize: '15px', fontWeight: 700, color: isDark ? '#FFFFFF' : '#0F172A' }}>+94 11 234 5678 / +94 77 123 4567</div>
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <div>
                    <div style={{ fontSize: '12px', color: isDark ? '#94A3B8' : '#64748B' }}>Official Email</div>
                    <div style={{ fontSize: '15px', fontWeight: 700, color: isDark ? '#FFFFFF' : '#0F172A' }}>support@edupulse.lk</div>
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: 'rgba(139, 92, 246, 0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <MapPin size={20} color="#8B5CF6" />
                  </div>
                  <div>
                    <div style={{ fontSize: '12px', color: isDark ? '#94A3B8' : '#64748B' }}>Headquarters</div>
                    <div style={{ fontSize: '15px', fontWeight: 700, color: isDark ? '#FFFFFF' : '#0F172A' }}>Edu Pulse Educational Technologies, Colombo 03, Sri Lanka</div>
                  </div>
                </div>
              </div>
              ...
              {/* Quick FAQs */}
              <div>
                <h4 style={{ fontSize: '17px', fontWeight: 700, marginBottom: '14px', color: isDark ? '#FFFFFF' : '#0F172A' }}>
                  {isSinhala ? 'නිතර අසන ප්‍රශ්න (FAQ)' : 'Frequently Asked Questions'}
                </h4>
                    ...
                    >
                      ...
                      >
                        ...
                        <ChevronDown
                          size={16}
                          style={{
                            transform: openFaq === fIdx ? 'rotate(180deg)' : 'none',
                      ...
                      {openFaq === fIdx && (
                        <div style={{ padding: '0 18px 14px', fontSize: '13px', color: isDark ? '#94A3B8' : '#64748B', lineHeight: 1.6 }}>
                          {faq.a}
                        </div>
                      )}
                    </div>
            ...
            <div style={{
              ...
              border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(0,0,0,0.08)',
              padding: '36px 30px',
              boxShadow: isDark ? '0 12px 30px rgba(0,0,0,0.3)' : '0 8px 24px rgba(15, 23, 42, 0.05)'
            }}>
              <h3 style={{ fontSize: '20px', fontWeight: 800, color: isDark ? '#FFFFFF' : '#0F172A', marginBottom: '8px' }}>
                {isSinhala ? 'අපට පණිවිඩයක් එවන්න' : 'Send Us a Direct Message'}
              </h3>
              <p style={{ fontSize: '14px', color: isDark ? '#94A3B8' : '#64748B', marginBottom: '24px' }}>
                {isSinhala
              ...
              {contactSubmitted ? (
                <div style={{
                  ...
                  <h4 style={{ fontSize: '17px', fontWeight: 700, color: '#10B981', marginBottom: '6px' }}>
                    {isSinhala ? 'පණිවිඩය සාර්ථකව ලැබිණි!' : 'Message Sent Successfully!'}
                  </h4>
                  <p style={{ fontSize: '13px', color: isDark ? '#CBD5E1' : '#475569', margin: 0 }}>
                    {isSinhala ? 'අපගේ කණ්ඩායම ඉක්මනින්ම ඔබ හා සම්බන්ධ වනු ඇත.' : 'Thank you for reaching out. We will get back to you shortly.'}
                  </p>
                </div>
              ) : (
                <form onSubmit={handleContactSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setContactForm({ ...contactForm, examTrack: e.target.value })}
                      style={{
                        width: '100%',
                        height: '44px',
                        borderRadius: '10px',
                    ...
                    >
                      <option value="g5">Grade 5 Scholarship (5 වසර ශිෂ්‍යත්වය)</option>
                      <option value="ol">G.C.E. Ordinary Level (O/L)</option>
                      <option value="al">G.C.E. Advanced Level (A/L)</option>
                      <option value="parent">Parent / Teacher / School License</option>
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      rows={4}
                      required
                      placeholder={isSinhala ? 'ඔබගේ ගැටලුව මෙහි සඳහන් කරන්න...' : 'How can our academic team help you?'}
                  ...
                  >
                    <Send size={16} />
                    <span>{contactSubmitting ? (isSinhala ? 'යවමින් පවතී...' : 'Sending...') : (isSinhala ? 'පණිවිඩය යවන්න' : 'Send Message')}</span>
                  </button>
                </form>
              )}
            </div>
      ...
      <footer style={{
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '36px', marginBottom: '48px' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
                ...
                <div style={{ fontSize: '18px', fontWeight: 800, color: isDark ? '#FFFFFF' : '#0F172A' }}>
                  Edu Pulse
                </div>
              </div>
              <p style={{ fontSize: '13px', color: isDark ? '#94A3B8' : '#64748B', lineHeight: 1.6, maxWidth: '280px' }}>
                {isSinhala
            ...
            <div>
              <div style={{ fontSize: '14px', fontWeight: 700, color: isDark ? '#FFFFFF' : '#0F172A', marginBottom: '14px' }}>
                Quick Navigation
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px' }}>
                <a href="#home" style={{ color: isDark ? '#94A3B8' : '#64748B', textDecoration: 'none' }}>Home</a>
                <a href="#features" style={{ color: isDark ? '#94A3B8' : '#64748B', textDecoration: 'none' }}>Features & Tools</a>
                <a href="#plans" style={{ color: isDark ? '#94A3B8' : '#64748B', textDecoration: 'none' }}>Plans & Pricing</a>
                <a href="#about" style={{ color: isDark ? '#94A3B8' : '#64748B', textDecoration: 'none' }}>About Edu Pulse</a>
                <a href="#previews" style={{ color: isDark ? '#94A3B8' : '#64748B', textDecoration: 'none' }}>Live Quiz Preview</a>
                <a href="#contact" style={{ color: isDark ? '#94A3B8' : '#64748B', textDecoration: 'none' }}>Contact & Help</a>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px' }}>
                <span style={{ color: isDark ? '#94A3B8' : '#64748B' }}>Grade 5 Scholarship IQ & Maths</span>
                <span style={{ color: isDark ? '#94A3B8' : '#64748B' }}>G.C.E. O/L Science & Mathematics</span>
                <span style={{ color: isDark ? '#94A3B8' : '#64748B' }}>G.C.E. O/L English, History & Commerce</span>
                <span style={{ color: isDark ? '#94A3B8' : '#64748B' }}>G.C.E. A/L Biological & Physical Science</span>
                <span style={{ color: isDark ? '#94A3B8' : '#64748B' }}>G.C.E. A/L Commerce & Arts Stream</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px' }}>
                <button
                  ...
                  style={{ background: 'none', border: 'none', padding: 0, textAlign: 'left', color: '#0284C7', fontWeight: 600, cursor: 'pointer' }}
                >
                  {user ? 'My Student Dashboard' : 'Student Login'}
                </button>
                <button
                  onClick={() => navigate('/login', { state: { isSignUp: true } })}
                  style={{ background: 'none', border: 'none', padding: 0, textAlign: 'left', color: isDark ? '#94A3B8' : '#64748B', cursor: 'pointer' }}
                >
                  Create New Account
                </button>
                <button
          ...
          <div style={{
            ...
            <div>
              © {new Date().getFullYear()} Edu Pulse Platform Sri Lanka. All rights reserved.
            </div>
