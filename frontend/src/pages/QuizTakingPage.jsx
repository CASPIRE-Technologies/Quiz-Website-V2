...
export default function QuizTakingPage() {
  ...
  const questions = [
    { id: 1, text: "Solve for x in the equation: 2x² - 8x + 6 = 0", options: ["x = 1 or x = 3", "x = -1 or x = -3", "x = 2 or x = 4", "x = 0 or x = 3"], correctIndex: 0 },
  ...
  const handleSubmit = async () => {
    const result = { score: 24, total: 30, percentage: 80, timeTaken: '32:45', date: new Date().toISOString().split('T')[0] };
    await api.submitAttempt(quizId, answers, 1965);
    addAttempt(quizId, result);
    navigate(`/quiz/${quizId}/result`);
  };

  const currQ = questions[currentIdx] || questions[0];
  ...
  return (
    <div style={{ margin: '-24px -24px 0 -24px' }}>
      ...
      <div style={{ height: '68px', backgroundColor: 'white', borderBottom: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', position: 'sticky', top: 0, zIndex: 50 }}>
        <div>
          <div style={{ fontSize: '14px', fontWeight: 700 }}>Algebra & Quadratic Equations Paper 01</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <button className="btn btn-danger btn-sm" onClick={handleSubmit}>Submit Quiz</button>
        </div>
      </div>
