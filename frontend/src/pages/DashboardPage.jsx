...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const scores = Object.values(attempts).map(
      (a) => a?.score ?? a?.percentage ?? 0,
    );
    const avgScore =
      scores.length > 0
    ...
    >
          ...
          <div>
            ...
            >
              ...
              <button
                ...
                onClick={() =>
                  navigate(
                    `/exams/${user?.examLevel?.includes("5") ? "g5" : user?.examLevel?.toLowerCase().includes("a") ? "al" : "ol"}`,
                  )
                }
                style={{
      ...
      >
        {[
          ...
          {
            label: t("dashboard.quizzesPurchased"),
      ...
      <div
        style={{
          background:
            "linear-gradient(135deg, var(--color-card-bg) 0%, var(--color-primary-light) 100%)",
          borderRadius: "18px",
          padding: "24px 28px",
          border: "1px solid var(--color-border)",
          display: "flex",
